...
En este capítulo se detalla cómo se ha diseñado el control de velocidad para el motor DC de las ruedas motrices del robot. Como ya se tienen calibrados los encoders para estimar la velocidad de las ruedas, se puede diseñar el control del motor para que siga una referencia de velocidad dada.
...
\begin{itemize}
	\item $U(s)$ corresponde a la consigna para la velocidad angular de la rueda ($w$ $rad/s$). La entrada se traduce en un valor de PWM que corresponde con un valor de voltaje que está limitado a un valor máximo de 6.5 V.
		...
		\caption{Ajuste lineal de la velocidad angular de las ruedas frente a la señal PWM}
...
Además, en la figura~\ref{fig:feedforward} se puede apreciar como las curvas de las distintas medidas tienden a la saturación a medida que se aumenta el valor de PWM. Este fenómeno se debe a causas mecánicas, como rozamiento, fricción y también a que se está próximo del voltaje máximo que acepta el motor. Otro factor a notar es la diferencia de las medidas de los diferentes robots y las diferencias entre las ruedas del mismo robot. Se puede ver que las medidas dadas por el robot 1 correspondiente a las figuras~\ref{fig:feedforwardR1_R}~\ref{fig:feedforwardrR1_left} son más uniformes y ambas ruedas tienen un comportamiento similar. El robot 3  tiene medidas más dispersas tal y como se aprecian en las figuras~\ref{fig:feedforwardR3_R}~\ref{fig:feedforwardrR3_left}. Además, se nota en las lecturas de la gráfica que los motores que las ruedas tienen diferentes respuestas y el robot 2 tiene lecturas más uniformes tal y como se aprecia en las figuras~\ref{fig:feedforwardR2_R}~\ref{fig:feedforwardrR2_left} que el robot 3 pero se puede apreciar una diferencia en las medidas obtenidas entre el motor izquierdo y el motor derecho. La respuesta de las gráficas demuestra que cada robot es diferente en cuanto a mediciones y movimiento. También se demuestra con este resultado que los motores de cada robot son muy distintos a pesar de venir del mismo fabricante y tener las mismas características. Otro factor notable en las gráficas es que el robot 3 tiene un valor menor de $w$  para el máximo PWM. Como el voltaje máximo es el mismo para todos, se deduce que el robot 3 tiene más rozamiento en las ruedas. Esto complica el diseño de un controlador genérico y que sirva para todos los robots. Por ello, se debe hacer un control de acción directa y un PID distinto para cada robot y rueda del robot.
\newpage
	\item $PID(z)$ corresponde al controlador proporcional, integral y derivativo. El control de acción directa no basta para mantener una velocidad igual a la de referencia. Además, el control de acción directa no responde adecuadamente a las variaciones debidas al ruido o perturbaciones. Es necesario un control realimentado que corrija el error y ajuste la salida al valor deseado. Para ello se usa uno de los controladores más conocidos que es el PID. Es un control que es proporcional al error, a su integral y a su derivada temporal. El control proporcional es una ganancia que se multiplica al error para llegar al valor de referencia y minimizar el error, tiene el problema de que se puede quedar en un valor próximo y nunca llegar al valor de consigna. El control integral sirve para integrar el error y como resultado corrige el error al cual no puede llegar el control proporcional. El control derivativo trata de anticiparse a un comportamiento futuro, este último control es muy sensible al ruido, y puesto que el robot tiene mucho ruido en la estimación de la velocidad, se ha decidido no aplicarlo.\\
	
	 Para la sintonización de los parámetros del PID se usa el método de ziegler-Nichols \cite{feedbacksystems}, que permite sintonizar los parámetros del controlador PID sin tener que conocer la planta. Esto sirve  para una primera aproximación y luego se va ajustando hasta conseguir una respuesta deseada. Además, de la sintonización de los parámetros del controlador, se implementa un módulo conocido como Anti-Wind-up.
	
	 \subitem El Wind-Up es un problema que se tiene con el control integral, relacionado con el tiempo que tarda el controlador en alcanzar el valor de consigna y la saturación de los actuadores. Hasta que no se alcance la consigna y el error no cambie de signo el valor de la integral del error sigue creciendo, y puede ocurrir que, una vez alcanzado el valor de consigna, el de la integral no sea cero, y el controlador sigue corrigiendo un error que no existe. Este fenómeno provoca una sobre-oscilación indeseada en el sistema, y puede llegar a inestabilizarlo en algunos casos. Debido a esto se propone dentro del control PID el denominado Anti-Wind-Up. Básicamente lo que hace es que cuando llega a un valor máximo el integrador deja de acumular valores y cuando se cambia el signo del error se borra la memoria del controlador.
...
\subsection{Sintonización de parámetros PID medainte el método de Ziglers-Nichols}
Debido al ruido del sistema, se decide no incluir la parte derivativa del controlador PID. Esto se debe a que la acción derivativa amplifica el ruido. Se podría poner un filtro a la acción derivativa, pero sería complicar el control de manera innecesaria y como se muestra a continuación con un control PI junto al control de acción directa es más que suficiente.\\
Se toma como ejemplo de sintonización el robot1, procediendo de modo análogo para el resto de robots.
Lo primero que se hace es aplicar una ganancia K en el lazo de control, se aumenta dicha ganancia hasta que se llegue a un comportamiento oscilatorio tal y como se aprecia en la figura~\ref{fig:respuestaEscalon}. Con la ganancia obtenida y el periodo de la oscilación resultante se hallan los parámetros del controlador PI.
\begin{figure}[htbp]
...
De la figura~\ref{fig:respuestaEscalon} se obtiene T que es el periodo de la oscilación. Para la rueda derecha $Td=0.59 ms$ que se obtiene tomando el tiempo entre dos máximos, tal y como se aprecia en la figura~\ref{fig:respuestaescalonrd} y para la rueda izquierda se tiene $Ti=0.55 ms$ en la figura~\ref{fig:respuestaescalonri} se tienen los máximos que se han usado. El valor de la ganancia para la que se ha conseguido la oscilación permanente del sistema es de K=2. A partir de los parámetros anteriores se obtienen los valores del Cuadro~\ref{tab:PID} que corresponden a las ganancias del control P, PI y PID.\\
 Para obtener la ganancia proporcional($K_{p}$) se multiplica a K por un factor menor a 1 que cambia, según el control sea solo proporcional (P), proporcional e integral (PI) o proporcional, integral y derivativo (PID). Para el control P se multiplica por 0.5, para PI  por 0.4 y para PID por 0.6. En el caso de la ganancia para el control integral($K_{i}$) se toma proporciones del periodo de oscilación, siendo para el control PI 0.8T y para PID 0.5T y por último para la ganancia del control derivativo($K_{d}$) se toma 0.125T como ganancia. \\
\begin{table}[h]
	\begin{center}
	\begin{tabular}{|c|c|c|c|c|c|c|}
		...
		\hline
	\end{tabular}
	\caption{Sintonización de controlador mediante el método de Ziglres-Nichols}
	\label{tab:PID} 
	\end{center}
\end{table}
Con las ganancias calculadas en el Cuadro~\ref{tab:PID} se vuelve a realizar una prueba para ver la respuesta de los motores. En la figura~\ref{fig:PI_Nichols} se puede apreciar la respuesta de los motores. Se puede ver que tanto en el motor derecho como en el izquierdo se tiene una oscilación además de una sobre-elongación grande.
...
\begin{figure}[h]
	...
	\begin{subfigure}[b]{0.52\linewidth}
		...
		\caption{Control PI rueda derecha\\
			Robot1}
		\label{fig:PI_R}
	\end{subfigure}
	\quad
		...
		\caption{Control PI rueda izquierda\\
			Robot1}
		\label{fig:PI_I}
	\end{subfigure}
	\caption{Control PI Zieglers-Nichols}
	\label{fig:PI_Nichols}
\end{figure}

 Se ajusta manualmente las ganancias del control PI y se llega al resultado mostrado en la figura~\ref{fig:PI_sintonizado}. Esta vez corrige de manera más eficaz, la sobre-elongación se mantiene, pero dura un breve instante de tiempo y la oscilación se ha reducido considerablemente. Se tiene una pequeña oscilación que se debe a parámetros mecánicos del robot, a la situación de las ruedas, alineación de los ejes, al propio ruido de los encoders y posiblemente a la cuantización del voltaje y no poder llegar a un valor exacto. Aun así el control es bastante efectivo.
 \begin{figure}[!h]
...
\begin{table}[!h]
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|}
			...
			\hline
		\end{tabular}
		\caption{Parámetros optimizados del controlador PI }
