...
\begin{itemize}
	...
	\item $G(s)$. Es la planta a controlar.
...
Ademas en las Figura~\ref{fig:feedforward} se puede apreciar como la curva tiende a la saturación a medida que se aumenta el valor de PWM, este fenómeno se debe a causas mecánicas, como rozamiento, fricción y también a que se está próximo del voltaje máximo que acepta el motor. Otro factor a notar es la diferencia de las medidas de los diferentes robot, se puede ver que las medidas dadas por el robot 1 son más uniformes que las otras, siendo el robot 3 el que tiene medidas más dispersas, uno pudiera pensar que se ha tenido un descuido al ir montando los diferentes robots, y esto no es así, pues el primero en montarse ha sido el robot2. La respuesta de las gráficas demuestra que cada robot es diferente en cuanto a mediciones y movimiento. Otro factor notable en las gráficas para realizar el control de acción directa es que el robot 3 tiene un valor menor de $w$  para un máximo PWM, como el voltaje máximo es el mismo para todos, lo que se deduce de esto es que el robot 3 tiene mas rozamiento en las ruedas, esto complica el diseñar un controlador genérico y que sirva para todos los robots, por ello se debe hacer un control de acción directa para cada robot y un PID distinto.
	...
	\item $H(s)$ es el sensor, que se refiere al encoder óptico, dicho encoder nos da pulsos que equivalen a $\theta=\frac{2\pi}{N}$ radianes, donde N=20, si se mide el tiempo entre pulsos se tiene la velocidad angular de la rueda.
\end{itemize}
...
De la representación gráfica se obtiene T, para la rueda derecha $TD=0.59$ que se obtiene tomando el tiempo entre dos máximos, tal y como se aprecia en la Figura~\ref{fig:respuestaescalonrd} y para la rueda izquierda se tiene $Ti=0.55$ en la Figura~\ref{fig:respuestaescalonri} se tienen los máximos que se han usado. Con estos valores y con k=2 ,se obtiene los valores del Cuadro~\ref{tab:PID}. Para la ganancia proporcional se toma proporciones de la K original, según sea P, PI o PID la proporción cambia, para P por ejemplo se toma 0.5K, para PI se toma 0.4K y para PID se toma 0.6K. En el caso de la ganancia para el control integral se toma proporciones del periodo de oscilación, siendo para el control PI 0.8T y para PID 0.5T y por último para la ganancia del control derivativo se toma 0.125T como ganancia. \\
\begin{table}[h]
	\begin{center}
	...
	\end{center}
\end{table}
Con las ganacias calculadas en el Cuadro~\ref{tab:PID} se vuelve a realizar una prueba para ver la respuesta de los motores. En la Figura~\ref{fig:PI_Nichols}, se puede apreciar la respuesta de los motores, se puede ver que en el motor derecho se tiene una oscilación y el motor izquierdo llega al asentamiento pero tiene una sobre-elongación grande.
