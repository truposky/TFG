...
\end{itemize}
Y en el robotario los componente correspondientes son los relacionados con la comunicación y la localización donde se ha buscado también que sea de fácil localización e implementación :
\begin{itemize}
...
\subsubsection{RaspberryPi y Picam}
	...
	\label{}
\end{figure}
	Se tiene una placa RaspberryPi4 montada en el robot para poder procesar imágenes adquiridas por una cámara Picam que va conectada a la RaspberryPi. Se alimenta directamente de la batería usando un conversor DC-DC para adaptar el voltaje a 5V que es el requerido por la raspberryPi y de esta manera,no tener que usar una segunda batería. La cámara conectada a la raspberryPi permite obtener imágenes con un angular de 160º según especificaciones del fabricante \cite{picam}. Este modulo no es esencial para el robotario pero lo hace más versátil pudiendo experimentar con algoritmos de localización a bordo, visión artificial,etc.
	
	\subsubsection{Montaje final de robots}
	\begin{figure}[!h]
		\centering
		\includegraphics[width=0.7\linewidth]{montajefinalR}
		\caption{Robots construidos}
		\label{fig:montajefinalr}
	\end{figure}
En la figura ~\ref{fig:montajefinalr} se tiene 3 robots montados, con los componentes anteriormente mencionados. Solo se dispone de 2 raspberryPi por ello uno no la lleva incorporada, pero con dos es suficiente para los requerimientos actuales del robotario. Se ha tenido que añadir una tercera planta donde se ha colocado el identificador de cada robot para que pueda ser localizado en el robotario, en la Figura ~\ref{fig:robotfinal} se tiene los robots montados con un identificador en la parte delantera y otro en la parte superior, para poder ser identificados y localizados por las cámaras.
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.5\linewidth]{robotfinal}
	\caption{Robots construidos con tercera planta}
...
Para la visión por computador se usa una librería denominada OpenCV \cite{opencv_library} la cual está desarrollada en dos lenguajes de programación c++ y Python. Permite reconocer el entorno con una cámara y tratar las imágenes para desarrollar la aplicación que se desee. En este proyecto se usa ArUco\cite{ArUco1}\cite{ArUco2} de OpenCV, que permite el reconocimiento de un Marker de tamaño 4x4 formado por 16 bits en la figura ~\ref{fig:ejemploMarker} se tiene un ejemplo de como son lo identificadores de los robots.
\subsubsection{Lenguaje de programación}
Todo el entrono de comunicación se ha desarrollado en C, con los sockets que se tienen en sistemas operativos Unix/linux y también con programación multi-hilo propia de sistemas Unix.\\
La placa de arduino se ha programado en c++ y en conjunto de las librerías que proporciona el entorno de arduino.
...
\end{center}
\end{table}
El componente más caro del robot es la raspberryPi. Por 134.99\euro, se tiene un robot que permite desarrollar diversos algoritmos en el entorno de pruebas,puede comunicarse y reconocer el entorno. Además el Robotario puede funcionar sin la raspberryPi y la cámara de Picam, pero esto limita el tipo de experimentos realizables. En total el coste de los robots del Robotario, con 5 robots adquiridos de los cuales 2 tiene RaspberryPi es de 452.83 \euro.\\
...
\begin{table}[h]
	...
	\end{center}
\end{table}
\\En total por 1121.83 \euro se tiene un Robotario funcional.

\subsection{Montaje final de Robotario}
Por último con los componentes adquiridos se ensambla el Robotario que se muestra en la figura ~\ref{fig:montajefinal}. Se coloca la cámara Logitec cámara cenital que sirve como sistema de localización global, la cámara se coloca encima del los límites del Robotario y lo mas centrada posible.
